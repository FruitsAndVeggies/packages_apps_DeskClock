 ...
 */
public class AnalogFNVAppWidgetProvider extends AppWidgetProvider {
    private static final boolean DEBUG = false;

    static final String TAG = "AnalogFNVAppWidgetProvider";
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        clockStyleDrawables = context.getResources().getStringArray(R.array.fnv_clock_backgrounds_drawable);

        for (int i=0; i<N; i++) {
            int selectedPos = AnalogFNVAppWidgetConfigure.loadClockPref(context, appWidgetIds[i]);
    ...
    public void onDeleted(Context context, int[] appWidgetIds) {
        ...
        final int N = appWidgetIds.length;
        for (int i=0; i<N; i++) {
            AnalogFNVAppWidgetConfigure.deleteClockPref(context, appWidgetIds[i]);
        }
    }
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(),
            R.layout.analog_fnv_appwidget);

        views.setOnClickPendingIntent(R.id.analog_fnv_appwidget,
